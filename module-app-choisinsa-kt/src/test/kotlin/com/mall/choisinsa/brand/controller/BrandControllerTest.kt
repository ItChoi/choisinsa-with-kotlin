        ...
        this.mockMvc.perform(get("/api/brands/with-item-count")
            ...
            .andDo(document("get_brands_with_item_count",
                requestFields(
                    ...
                    fieldWithPath("companyId").type(JsonFieldType.NUMBER).description("회사 id").optional(),
                ...
                responseFields(
                    ...
                    fieldWithPath("data.[].nameEn").type(JsonFieldType.STRING).description("브랜드 영문명"),
                    fieldWithPath("data.[].nameKo").type(JsonFieldType.STRING).description("브랜드 국문명"),
                    fieldWithPath("data.[].totalItemCount").type(JsonFieldType.NUMBER).description("총 아이템 개수"),
                )
            ))
    }

    @Test
    @DisplayName("모든 브랜드를 조회한다. 브랜드 정보만 포함 (브랜드 상세 등 제외)")
    fun getSimpleBrands() {
        // given
        val search = BrandSearch(
            listOf(1L, 2L, 3L),
        ...
        val responses = listOf(SimpleBrandResponse(1L, 1L, "칼하트", "칼하트 반팔티", "칼하트 반팔티"))

        // when
        given(brandService.findAllBrandResponseBy(search))
            .willReturn(responses)
        ...
        this.mockMvc.perform(get("/api/brands")
            ...
            .content(objectMapper.writeValueAsString(search)))
            .andExpect(status().isOk)
            .andDo(document("get_simple_brands",
                requestFields(
                    fieldWithPath("brandIds").type(JsonFieldType.ARRAY).description("브랜드 ids").optional(),
                    fieldWithPath("companyId").type(JsonFieldType.NUMBER).description("회사 id").optional(),
                    fieldWithPath("brandNameEn").type(JsonFieldType.STRING).description("브랜드 영문명").optional(),
                ...
                responseFields(
                    ...
                    fieldWithPath("data.[].nameEn").type(JsonFieldType.STRING).description("브랜드 국문명"),
                    fieldWithPath("data.[].nameKo").type(JsonFieldType.STRING).description("브랜드 영문명"),
                )
            ))
